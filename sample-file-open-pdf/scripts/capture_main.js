...
captureApp.prototype = {
    ...
    _captureSuccess:function(capturedFiles) {
        ...
        for (i=0;i < capturedFiles.length;i+=1) {
            media.innerHTML+='<p>Capture taken! Its path is: ' + capturedFiles[i].fullPath + '</p><br />' + '<span id="srcPath">' + capturedFiles[i].fullPath + '</span>'
        } 
        var getSpan = document.getElementById("srcPath").innerHTML;
        localStorage.setItem("capturePath", getSpan); 
    },
